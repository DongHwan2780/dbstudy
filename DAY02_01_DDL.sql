...
(
    ...
    CUST_PH VARCHAR2(30 BYTE) UNIQUE,
    CUST_AGE NUMBER(3) CHECK(CUST_AGE >= 0 AND CUST_AGE <= 120),
                       -- CHECK(CUST_AGE >= 0 AND CUST_AGE <= 120) == CHECK(CUST_AGE BETWEEN 0 AND 120)         
...
/*
    테이블 수정하기
    1. 칼럼 추가하기  :   ALTER TABLE 테이블명 ADD 칼럼명 데이터 타입 제약조건
    2. 칼럼 정보 수정하기   :   ALTER TABLE 테이블명 MODIFY 칼럼명 데이터 타입 제약조건
...
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(30 BYTE) UNIQUE;

-- 2. BANK_T의 은행명(BANK_NAME) 칼럼을 VARCHAR2 15바이트로 변경하고 필수 제약 조건 추가
ALTER TABLE BANK_T MODIFY BANK_NAME VARCHAR2(15 BYTE) NOT NULL;

-- 3. CUSTOMER_T의 나이 칼럼을 삭제
ALTER TABLE CUSTOMER_T DROP COLUMN CUST_AGE;

-- 4. CUSTOMER_T의 연락처 칼럼명을 CUST_TEL로 수정하시오
ALTER TABLE CUSTOMER_T RENAME COLUMN CUST_PH TO CUST_TEL;

-- 5. CUSTOMER_T에 등급 칼럼을 추가 / VIP GOLD SILVER BRONZE 중 하나의 값을 가지도록
ALTER TABLE CUSTOMER_T ADD CUST_GRADE VARCHAR2(15 BYTE) CHECK(CUST_GRADE = 'VIP' OR CUST_GRADE = 'GOLD' OR CUST_GRADE = 'SILVER' OR CUST_GRADE = 'BRONZE');
-- CHECK(CUST_GRADE = 'VIP' OR CUST_GRADE = 'GOLD' OR CUST_GRADE = 'SILVER' OR CUST_GRADE = 'BRONZE') == CHECK(CUST_GRADE IN('VIP','GOLD','SILVER','BRONZE'));

-- 6. CUSTOMER_T의 고객명 칼럼의 필수 제약 조건을 삭제
ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;

-- 7. CUSTOMER_T의 테이블명을 CUST_T로 변경
...
/*
    ...
    1. PK
        1) 추가 : ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건이름 PRIMARY KEY(칼럼명)
        2) 삭제
            (1) ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건이름 
            (2) ALTER TABLE 테이블명 DROP PRIMARY KEY
    2. FK
        1) 추가 : ALTER TABLE 자식테이블명 ADD CONSTRAINT 제약조건이름 FOREIGN KEY(칼럼명) REFERENCES 부모테이블명(참조할 칼럼) [옵션]
