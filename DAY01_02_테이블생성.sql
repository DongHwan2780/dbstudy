...
/*
    ...
    1. NUMBER(p, s) : 정밀도가 P, 스케일이 S인 숫자 
        1) 정밀도 P : 전체 유효 숫자 = 숫자 전체 길이
        2) 스케일 S : 소수부의 유효 숫자 = 소숫점 길이
        3) 스케일만 생략하면 정수로 표시하는 숫자
        4) 정밀도와 스케일을 모두 생략 가능 -> 정수, 실수 모두 표시할 수 있는 숫자
    2. CHAR(size) : 글자수가 최대 size인 고정 문자 타입
        1) 고정 문자 : 글자수의 변동이 적은 문자(예시 : 폰 번호, 주민번호 등)
        2) 최대 size : 2000 byte
...
/*
    제약조건 
    1. NOT NULL : 필수 입력 -> 필수가 아닐 경우 생략 가능
    2. UNIQUE : 중복 불가
    3. PRIMARY KEY : 기본 키 == 식별자
    4. FOREIGN KEY : 외래 키
...
CREATE TABLE BLOG_T 
(
    BLOG_NUM NUMBER NOT NULL PRIMARY KEY,
    TITLE VARCHAR2(128 BYTE) NOT NULL,
    EDITOR VARCHAR2(32 BYTE) NOT NULL,
    CONTENTS CLOB NULL,
